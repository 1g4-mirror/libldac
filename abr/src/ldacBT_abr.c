...
{
    int flagPriorityChanged, abrQualityModeID, encQModeID, i;
    ...
    abrQualityModeID = -1;
    if ((LDACBT_EQMID_HQ <= encQModeID) &&
            (encQModeID < (int)sizeof(aEqmidToBitrateSortedID))) {
...
#endif
    if (flagEnable) {
        if (flagPriorityChanged) {
            int abrQualityModeIDNew;
            if (flagPriorityChanged < 0) {
                for (i = 0; i > flagPriorityChanged; --i) {
                ...
                if (encQModeID >= 0) {
                    if (encQModeID < (int)sizeof(aEqmidToBitrateSortedID)) {
