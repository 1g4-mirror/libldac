 ...
 *
 * Notes on using LDAC ABR:
 * It is necessary to confirm whether LDAC ABR algorithm works fine on the final product.
 ...
 *
 * It is able to confirm the bit rate transition by checking logcat messages from LDAC ABR library
 * which built with the following changes in Android.bp:
 ...
 *     "ldacBT_abr.c".
 *
 * And It is also able to estimate the bit rate transition by listening to the audio played on the
 * SNK device. This method cannot use to confirm the details of the bit rate transition, but useful
